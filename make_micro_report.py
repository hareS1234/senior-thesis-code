#!/usr/bin/env python3
"""Gather the microscopic kinetic checks into CSV and text reports."""
...
def read_min_set(path: Path) -> np.ndarray:
    """Read min.A/min.B, with or without the leading count."""
    if not path.exists():
...
def load_AB_selectors(markov_dir: Path, dps_dir: Path, tag: str) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    """Use saved A/B selectors when possible, otherwise rebuild the mapping."""
...
def load_mfpt_npz(markov_dir: Path, tag: str) -> Dict[str, float]:
    """Load the MFPT archive, including the older key names."""
    out: Dict[str, float] = {}
...
def infer_system_label(root: Path, markov_dir: Path) -> str:
    """Use the first folder under root as the system label."""
